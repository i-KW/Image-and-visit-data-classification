...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        if self.augument:
            X = self.augumentor(X) #对图片作数据增强
        X = T.Compose([T.ToPILImage(),T.ToTensor()])(X)
        # visit=T.Compose([T.ToTensor()])(visit)
        return X.float(),y  #返回的是图片、访问数据以及label


    def read_images(self,index):  #读取图片数据的函数
    ...
    def augumentor(self,image):  #数据增强变换器
        augment_img = iaa.Sequential([ #Sequential(C, R)选取一系列子增强器C作用于每张图片，第二个参数表示是否对每个batch的图片应用不同顺序的Augmenter list。当设置为True时，不同batch之间图片的处理顺序都会不一样，但是同一个batch内顺序相同
            iaa.Fliplr(0.5),
            iaa.Flipud(0.5),
            iaa.SomeOf((0,4),[  #将Augmenter中的部分变换应用在图片处理上，而不是应用所有的Augmenter
            ...
            iaa.OneOf([  #每次从一系列Augmenters中选择一个来变换
                    ...
                    iaa.AverageBlur(k=(2, 7)), # 从最邻近像素中取均值来扰动，均值模糊。blur image using local means with kernel sizes between 2 and 7
                    iaa.MedianBlur(k=(3, 11)), # 通过最近邻中位数来扰动，中值模糊。blur image using local medians with kernel sizes between 2 and 7
                ]),
...
class MultiModalNet(nn.Module):
    def __init__(self, backbone1, backbone2, drop, pretrained=True):
        ...
        if drop > 0:
            self.img_fc = nn.Sequential(FCViewer(),
                                    nn.Dropout(drop),
                                    nn.Linear(img_model.last_linear.in_features, 256)) #256为图片输出的向量维度
        ...
        self.cls = nn.Linear(320,config.num_classes) #320为图片向量和visit数据向量合并后的向量维度
    ...
    def forward(self, x_img,x_vis):
        ...
        # x_cat = torch.cat((x_img,x_vis),1) #不需要concatnate向量
